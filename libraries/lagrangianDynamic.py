...
class LagrangeDynamic:
    def __init__(self, thetas, fk: FK_Exponential, fkc: FK_Exponential, masses, gravity):
        ...
        self.velocities = self.__compute_velocities()  # Compute velocities for each link
        self.inertia_array = self.__generate_intertia_matrices()  # Generate symbolic inertia matrices for each link
    ...
    def __generate_intertia_matrices(self):
        ...
        for i in range(self.num_joints):
            I = sp.Matrix([[sp.symbols(f'Ixx{i+1}'), sp.symbols(f'Ixy{i+1}'), sp.symbols(f'Ixz{i+1}')],
    ...
    def __compute_velocities(self):
        ...
        jacobian_matrices = self.fk.get_jacobian_matrices()  # Get Jacobian matrices for each link
    ...
    def __compute_massPoint_v_squared_all_joints(self):
        ...
        for i in range(self.num_joints):
            v = self.velocities[i]
            v_squared = (v.T * v)[0]  # Compute the squared magnitude of the velocity vector
    ...
    def __compute_jacobian(self):
        ...
        """
        transformation_matrices = self.fk.get_transformation_matrices()  # Get transformation matrices for each link
        transformation_matrices_c = self.fkc.get_transformation_matrices()  # Get transformation matrices for each link's center of mass
        Jv_mtx = []
        Jw_mtx = []
        for i in range(1, self.num_joints + 1):
            Jv = sp.zeros(3, self.num_joints)
            Jw = sp.zeros(3, self.num_joints)
            oc_i = sp.Matrix(transformation_matrices_c[i])[:3, 3]  # Position of the center of mass of link i
            for j in range(i):
                ...
                jv = z_j.cross(oc_i - o_j)  # pyright: ignore[reportOperatorIssue] # Linear velocity Jacobian component
    ...
    def __compute_total_massPoint_K(self):
        ...
        total_v_squared = 0
        for m, v_squared in zip(self.masses, self.MassPoint_v_squared_all_joints):
            mv_squared = m * v_squared  # Compute the kinetic energy contribution of each link
            
            total_v_squared += mv_squared
        
        total_v_squared = sp.simplify(total_v_squared)  # Simplify the total squared velocity expression

        kinetic_energies = sp.Rational(1, 2) * total_v_squared  # Compute the total kinetic energy of the manipulator

        return sp.trigsimp(kinetic_energies)
    ...
    def __compute_total_Lagrange_Dv(self):
            ...
            Dv = sp.zeros(self.num_joints, self.num_joints)  # Initialize the total kinetic energy
            # total_m_Jv_sq = 0
            for m, jv in zip(self.masses, Jv):
                jv = sp.Matrix(jv)
                mJvJv = m * (jv.T * jv) # Compute the kinetic energy contribution of each link
                Dv += mJvJv
    ...
    def __compute_total_Lagrange_Dw(self):
        ...
        total_JwRIRJw = sp.zeros(self.num_joints, self.num_joints)  # Initialize the total kinetic energy
        for i in range(self.num_joints):
            jw = sp.Matrix(Jw[i])
            R = sp.Matrix(self.fk.get_rotation_matrices()[i])  # Get the rotation matrix for the current link
            I = sp.Matrix(self.inertia_array[i])
            RIR = R * I * R.T  # Rotate the inertia matrix to the world frame
            JwiJw = jw.T * RIR * jw
            total_JwRIRJw += JwiJw
        Dw = sp.simplify(total_JwRIRJw)  # Compute the total kinetic energy of the manipulator
        return Dw
    ...
    def __compute_Dq(self):
        ...
        '''
        if self.fkc:
            Dv = self.__compute_total_Lagrange_Dv()  # Compute the total kinetic energy of the manipulator
            Dw = self.__compute_total_Lagrange_Dw()  # Compute the total kinetic energy of the manipulator
            Dq = Dv + Dw
        else:
            tau = sp.Matrix(self.torques)
            Dq = tau.jacobian(sp.Matrix(self.qddot))  # Compute the Jacobian of tau with respect to qddot
    ...
    def __compute_Cq(self, Dq):
        ...
        Cijk = sp.MutableDenseNDimArray.zeros(self.num_joints, self.num_joints, self.num_joints)  # Initialize the Coriolis and centrifugal forces
        for i in range(self.num_joints):
            for j in range(self.num_joints):
                for k in range(self.num_joints):
                    Cijk[i, j, k] = (sp.diff(D[i, j], self.thetas[k]) + sp.diff(D[i, k], self.thetas[j]) - sp.diff(D[j, k], self.thetas[i])) / 2 # pyright: ignore[reportOperatorIssue]

        for i in range(self.num_joints):
            for j in range(self.num_joints):
                C[i, j] = sum(Cijk[i, j, k] * self.qdot[k] for k in range(self.num_joints))  # Compute the Coriolis and centrifugal forces
    ...
    def __compute_Gq(self):
        ...
        for i in range(self.num_joints):
            G[i] = sp.diff(self.Potential_energy_P, self.thetas[i])  # Compute the gravitational forces
    ...
    def __compute_potential_energies(self):
        ...
        potential_energies = []
        if self.fkc:
            transformation_matrices = self.fkc.get_transformation_matrices()[1:] # Get transformation matrices for each link's center of mass
        else:
            transformation_matrices = self.fk.get_transformation_matrices()[1:] # Get transformation matrices for each link

        for i in range(self.num_joints):
            T = sp.Matrix(transformation_matrices[i])  # Get the transformation matrix for the current link
            z = sp.sympify(T[2, 3])  # Extract the scalar z-position from the transformation matrix
    ...
    def __compute_total_potential_energy(self):
        ...
        for P in self.potential_energies:
            total_potential_energy += sp.trigsimp(P)
        return sp.trigsimp(total_potential_energy)
    ...
    def __compute_differential_L(self):
        ...
        dL_dqdot_dt = dL_dqdot.jacobian(q) * qdot + dL_dqdot.jacobian(qdot) * qddot  # Compute the time derivative using the chain rule
    ...
    def __compute_torques(self):
        ...
        torques = self.dL_qdot_dt - self.dL_q # Compute the torques: tau = d/dt(dL/dqdot) - dL/dq
        return sp.simplify(torques)

    def __get_DCG_matrices(self):
        tau = sp.Matrix(self.torques)
        D = tau.jacobian(sp.Matrix(self.qddot))  # Compute the Jacobian of tau with respect to qddot
        D = sp.simplify(D)
        zero_motion = {
        ...
        C = tau - D * sp.Matrix(self.qddot) - G  # Compute C(q, qdot) = tau - D(q) * qddot - G(q)
        C = C.jacobian(sp.Matrix(self.qdot))  # Compute the Jacobian of C with respect to qdot
        C = sp.simplify(C)
        return D, C, G
